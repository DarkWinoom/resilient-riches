...
- 日历 **无点＝未录入，单点＝部分分类已录，双点＝当天全部可录入分类已录**。这是记录状态，不是每天必须完成的任务。当前分类启用日之前的日期不可选，并会提示启用日期。
- 记录顶部显示启用日期与累计盈亏金额；修改核对金额、买入或卖出时，累计盈亏和当日收益预览实时更新。切换日期时保留内容，以加载指示提示进度。
- 未录入日期沿用上次余额；周末或没有变动时可以跳过。再次核对的余额差额计入本次日期。
- 金额清空会归零，最多两位小数，`200.` 也会按 `200` 处理；历史盈亏可填负数。非法金额会立即提示并阻止输入。
...
分类持仓左侧可拖动排序，松开后自动保存。已清仓分类默认显示，可通过“隐藏已清仓”切换。

分类详情独立切换年、月、周、日，默认查看年；年/月展示收益曲线，周/日展示每天的盈亏。曲线使用具体日期，并在启用日计入历史盈亏；初始资金是计算本金，不作为收益。

在“编辑”中选择“一键清仓”，填写本轮最终盈亏并确认，当前金额即归零；本轮累计盈亏以填写的结算结果为准。重新激活会沿用名称、颜色与备注，重新设置启用日期和初始资金，历史盈亏默认填上次清仓金额且可修改；保存后以本次填写的金额作为累计起点，旧清仓结果不再额外累加。删除分类会同时删除该分类的全部轮次记录，操作前会显示影响范围并再次确认。
...
收益日报及其分享图片均不显示曲线；所选期间没有实际记录时，分享暂不可用。录入了零收益记录仍可分享。备注与简评不会出现在分享图中。
...
备份包含已提交的全部记录，服务运行中也可执行。请把备份另存到其他磁盘。v1.1.0 启动时会将旧版数据库升级到 schema 2。更新应保留同一个数据卷；如需回退 v1.0.0，请停止服务并恢复升级前备份。
...
组合历史因子按已启用分类的初始金额与历史本金合计计算；历史数据作为一次初始收益衔接，不推算历史每日走势。日／周／月／年期间摘要统计实际期间变动；收益曲线额外将历史盈亏记在启用日，周期不包含启用日时不会重复加入。收益率按每日资金基数计算并复利连乘，修改历史资金或记录后会重新计算受影响的日、周、月、年结果。
