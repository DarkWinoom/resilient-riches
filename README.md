...
**v1.0.0** · 自定义分类记账 · 人民币 · 单人私有部署 · [MIT 开源](LICENSE)

按“稳健理财”“债券投资”“长期成长”等分类核对余额、记录买入卖出，自动计算盈亏。无需逐个维护具体产品，也不必每天录入。

[部署](#部署) · [开始记账](#开始记账) · [报表分享](#报表与分享) · [更新与备份](#更新与备份) · [常见问题](#常见问题)

![首页：金额总览、收益曲线、资金分布与分类持仓](assets/screenshots/overview.jpg)

所有截图均为模拟账本。首次部署得到空账本，数据保存在你自己的 SQLite 数据库中。
...
采用 [MIT License](LICENSE)，允许使用、修改、分发和商业使用，请保留版权与许可文本。第三方依赖遵循各自许可，见[第三方声明](apps/web/public/third-party-notices.txt)。
