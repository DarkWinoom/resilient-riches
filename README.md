...
- **记录盈亏**：填写当日余额、转入和转出，实时预览盈亏。悬停右上按钮可直接进入单分类；修改后才能保存，补录历史日期时保存后继续录入。
...
初始资金是启用时的实际余额，已包含历史盈亏。历史本金按“初始资金−历史盈亏”还原；无法还原的历史部分保留盈亏金额，但不计入百分比，并在界面注明。历史金额不会使有效的后续收益率曲线变空。
