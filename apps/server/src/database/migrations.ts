...
export const migrations: readonly Migration[] = [
  ...
  {
    version: 2,
    name: 'holding-cycles-and-liquidation',
    sql: `
      CREATE TABLE categories_v2 (
        id TEXT PRIMARY KEY NOT NULL CHECK (length(id) BETWEEN 1 AND 64),
        name TEXT NOT NULL COLLATE NOCASE CHECK (length(trim(name)) BETWEEN 1 AND 40 AND name = trim(name)),
        color TEXT NOT NULL CHECK (length(color) = 7 AND substr(color, 1, 1) = '#' AND substr(color, 2) NOT GLOB '*[^0-9a-fA-F]*'),
        opening_date TEXT NOT NULL CHECK (length(opening_date) = 10 AND date(opening_date) IS NOT NULL AND date(opening_date) = opening_date),
        opening_balance_minor INTEGER NOT NULL CHECK (opening_balance_minor BETWEEN 0 AND 99999999999999),
        historical_pnl_minor INTEGER NOT NULL CHECK (historical_pnl_minor BETWEEN -99999999999999 AND 99999999999999),
        ...
        sort_order INTEGER NOT NULL DEFAULT 0,
        archived_on TEXT CHECK (archived_on IS NULL OR (length(archived_on) = 10 AND date(archived_on) IS NOT NULL AND date(archived_on) = archived_on AND archived_on >= opening_date)),
        previous_cycle_id TEXT REFERENCES categories_v2(id) ON DELETE SET NULL,
        revision INTEGER NOT NULL DEFAULT 1 CHECK (revision BETWEEN 1 AND 2147483647),
        created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
        updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
      ) STRICT;
      CREATE TABLE daily_entries_v2 (
        id TEXT PRIMARY KEY NOT NULL CHECK (length(id) BETWEEN 1 AND 64),
        category_id TEXT NOT NULL REFERENCES categories_v2(id) ON DELETE CASCADE,
        date TEXT NOT NULL CHECK (length(date) = 10 AND date(date) IS NOT NULL AND date(date) = date),
        closing_balance_minor INTEGER NOT NULL CHECK (closing_balance_minor BETWEEN 0 AND 99999999999999),
        buy_minor INTEGER NOT NULL CHECK (buy_minor BETWEEN 0 AND 99999999999999),
        sell_minor INTEGER NOT NULL CHECK (sell_minor BETWEEN 0 AND 99999999999999),
        note TEXT NOT NULL DEFAULT '' CHECK (length(note) <= 1000),
        liquidation_pnl_minor INTEGER CHECK (liquidation_pnl_minor BETWEEN -99999999999999 AND 99999999999999),
        revision INTEGER NOT NULL DEFAULT 1 CHECK (revision BETWEEN 1 AND 2147483647),
        created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
        updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
        UNIQUE (category_id, date)
      ) STRICT;
      INSERT INTO categories_v2 (id,name,color,opening_date,opening_balance_minor,historical_pnl_minor,note,sort_order,archived_on,revision,created_at,updated_at)
        SELECT id,name,color,opening_date,opening_balance_minor,historical_pnl_minor,note,sort_order,archived_on,revision,created_at,updated_at FROM categories;
      INSERT INTO daily_entries_v2 (id,category_id,date,closing_balance_minor,buy_minor,sell_minor,note,revision,created_at,updated_at)
        SELECT id,category_id,date,closing_balance_minor,buy_minor,sell_minor,note,revision,created_at,updated_at FROM daily_entries;
      ...
      CREATE UNIQUE INDEX category_next_cycle ON categories(previous_cycle_id) WHERE previous_cycle_id IS NOT NULL;
      CREATE INDEX daily_entries_date ON daily_entries(date);
