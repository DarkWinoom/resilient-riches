...
import { rateLabel, moneyLabel, signedMoney } from './format.ts';
...
export function renderShareSvg(model: ShareModel): { svg: string; width: number; height: number } {
  ...
  const height = rows ? tableTop + tableHeight + 125 : model.amounts ? 800 : 760;
  const parts = [
    ...
    text(98, 109, '稳健生财', 32, '#302d27', 'start', 600),
    text(828, 107, reportTitles[model.period], 28, '#9a742e', 'end', 500),
    text(
      72,
      158,
  ...
  if (model.current) parts.push(text(828, 158, `截至 ${model.range.to}`, 22, '#827a6e', 'end'));
  parts.push(`<rect x="70" y="186" width="760" height="134" rx="14" fill="#f7f5ef"/>`);
  if (model.amounts) {
    parts.push(
      text(94, 224, '本期收益', 23, '#827a6e'),
      text(94, 271, signedMoney(model.amounts.pnl), 38, tone(model.amounts.pnl), 'start', 600),
      text(465, 224, '复利收益率', 23, '#827a6e'),
      text(465, 271, rateLabel(model.returnRate), 38, tone(model.returnRate), 'start', 600),
    );
  } else
    parts.push(
      text(450, 224, '本期复利收益率', 24, '#827a6e', 'middle'),
      text(450, 282, rateLabel(model.returnRate), 48, tone(model.returnRate), 'middle', 600),
    );
  ...
  const y = (value: number) => top + 20 + ((high - value) / (high - low)) * (chartHeight - 52);
  ...
  if (model.amounts)
    parts.push(
      text(72, 654, `期末资产  ¥ ${moneyLabel(model.amounts.endingBalance)}`, 24, '#827a6e'),
    );
  if (rows) {
    parts.push(
  ...
  parts.push(
    ...
    text(828, height - 68, '@2026 by DarkWinoom', 22, '#827a6e', 'end'),
    '</g></svg>',
  );
